...
#Create X matrix of predictors with ln-transformed continuous variables (PCBs, Dioxins, and Furans) and dummy variables for those that are categorical.
x <- model.matrix(telomean ~ ., studypop)[,-1]

#ln transform x
x[,1:8] <- log(x[,1:8])
x[,15:24] <- log(x[,15:24])
#grpreg standardizes the data for us
...
#Cross-validation (CV) to choose the best tuning parameter lambda. If not specified in code, ten-fold CV is the default.
...
#4 group model
#Use CV to choose the best tuning parameter lambda (ten-fold default)
...
```{r vizlasso}
#3 group model
lasso_beta_3 <- cbind(rownames(fit_lasso_3$beta), fit_lasso_3$beta) %>% 
  as.tibble() %>% 
...
lasso_beta_3 %>% filter(group3 %in% c("TEQ", "Non-Dioxin-like PCB", "Non-Ortho PCB")) %>% 
...
#4 group model
lasso_beta_4 <- cbind(rownames(fit_lasso_4$beta), fit_lasso_4$beta) %>% 
  as.tibble() %>% 
...
```{r vizmcp}
#3 group model
mcp_beta_3 <- cbind(rownames(fit_mcp_3$beta), fit_mcp_3$beta) %>% 
  as.tibble() %>% 
...
#Filter out covariates
mcp_beta_3 %>% filter(group3 %in% c("TEQ", "Non-Dioxin-like PCB", "Non-Ortho PCB")) %>% 
  ggplot(aes(x = variable, y = round(beta, 5))) + 
  geom_point() + theme_minimal() + geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  facet_wrap(~ group3, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip() + 
...
#4 group model
mcp_beta_4 <- cbind(rownames(fit_mcp_4$beta), fit_mcp_4$beta) %>% 
  as.tibble() %>% 
...
```{r vizscad}
#3 group model
scad_beta_3 <- cbind(rownames(fit_scad_3$beta), fit_scad_3$beta) %>% 
  as.tibble() %>% 
...
#Filter out covariates
scad_beta_3 %>% filter(group3 %in% c("TEQ", "Non-Dioxin-like PCB", "Non-Ortho PCB")) %>% 
  ggplot(aes(x = variable, y = round(beta, 5))) + 
  geom_point() + theme_minimal() + geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  facet_wrap(~ group3, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip() + 
...
#4 group model
scad_beta_4 <- cbind(rownames(fit_scad_4$beta), fit_scad_4$beta) %>% 
  as.tibble() %>% 
...
scad_beta_4 %>% 
  filter(group4 %in% c("Dioxin", "Furan", "Non-Dioxin-like PCB", "Non-Ortho PCB")) %>% 
  ggplot(aes(x = variable, y = round(beta, 5))) + geom_point() +
...
studypop <- studypop %>% mutate(logtelomean = log(telomean))

lm_from_mcp <- lm(logtelomean ~ lbx118la + lbxpcbla +  lbxhxcla + 
     lbxwbcsi + lbxlypct + lbxmopct + lbxnepct + lbxeopct + lbxbapct +
     as.factor(bmi_cat3) + as.factor(edu_cat) + as.factor(race_cat) + male, data = studypop)

lm_from_mcp %>% broom::tidy() %>% mutate(
  estimate = round(estimate, 4),
  std.error = round(std.error, 4),
  statistic = round(statistic, 4),
  p.value = round(p.value, 4))

lm_from_mcp %>% broom::glance() %>% mutate(p.value = round(p.value, 5))
```

### Plot Model

```{r}
lm_from_mcp %>% broom::tidy() %>% as.tibble() %>% filter(grepl("la", term)) %>% 
  ...
  labs(y = "Estimate", x = "Pollutant", title = "Association between Pollutants from MCP and LTL")
