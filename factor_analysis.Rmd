...
fit <- as.data.frame(rbind(cbind("3 Factor", "Varimax", round(fa_3$BIC), round(fa_3$TLI, 4),
      ...
      cbind("6 Factor", "Varimax", round(fa_6$BIC), round(fa_6$TLI, 4), 
            round(unname(fa_6$RMSEA[1]), 4)),
      ...
      cbind("6 Factor", "Promax", round(fa_6_p$BIC), round(fa_6_p$TLI, 4), 
            round(unname(fa_6_p$RMSEA[1]), 4))))
...
```{r}
loadings <- as.tibble(cbind(rownames(fa_6$loadings[]), fa_6$loadings[])) %>% 
  rename(Variable = V1) %>% 
