...
Perform an eigenvalue analysis to get an estimate (or range) for the number of underlying dimensions. Recommend fitting as least one more and one less factor than suggested by the eigenvalue analysis.
...
fa_6 <- fa(logstudypop, 
                 nfactors = 6, n.obs = 1003,
                 rotate = "varimax", #varimax is an orthogonal rotation
                 scores = "regression")

print(fa_6, digits = 2, sort = TRUE)
...
fa_6_p <- fa(logstudypop, 
                 nfactors = 6, n.obs = 1003,
                 rotate = "promax",
                 scores = "regression")

print(fa_6_p, digits = 2, sort = TRUE)
...
```{r}
fit <- as.data.frame(rbind(cbind("3 Factor", "Varimax", round(fa_3$BIC), round(fa_3$TLI, 4),
                                 round(unname(fa_3$RMSEA[1]), 4)),
      cbind("4 Factor", "Varimax", round(fa_4$BIC), round(fa_4$TLI, 4), round(unname(fa_4$RMSEA[1]), 4)),
      cbind("5 Factor", "Varimax", round(fa_5$BIC), round(fa_5$TLI, 4), round(unname(fa_5$RMSEA[1]), 4)),
      cbind("6 Factor", "Varimax", round(fa_6$BIC), round(fa_6$TLI, 4), round(unname(fa_6$RMSEA[1]), 4)),
      cbind("3 Factor", "Promax", round(fa_3_p$BIC), round(fa_3_p$TLI, 4), round(unname(fa_3_p$RMSEA[1]), 4)),
      cbind("4 Factor", "Promax", round(fa_4_p$BIC), round(fa_4_p$TLI, 4), round(unname(fa_4_p$RMSEA[1]), 4)),
      cbind("5 Factor", "Promax", round(fa_5_p$BIC), round(fa_5_p$TLI, 4), round(unname(fa_5_p$RMSEA[1]), 4)),
      cbind("6 Factor", "Promax", round(fa_6_p$BIC), round(fa_6_p$TLI, 4), round(unname(fa_6_p$RMSEA[1]), 4))))
...
Choose Varimax 6 factor model based on fit statistics.

```{r}
loadings <- as.tibble(cbind(rownames(fa_6$loadings[]), fa_6$loadings[])) %>% rename(Variable = V1) %>% 
...
loadings$Max <- colnames(loadings[,2:7])[max.col(loadings[,2:7], ties.method="first")]
...
loadings %>% filter(Max == "MR6") %>% count()

8 + 4 + 3 + 2 + 1 

scores <- as.tibble(cbind(rownames(fa_6$scores[]), fa_6$scores[]))
...
scores %>% filter(Max == "MR6") %>% count()

189 + 159 + 169 + 164 + 125 + 197
```

## Data Viz

```{r}
loadings <- loadings %>% mutate(Group = ifelse(Variable == "lbx118la", "TEQ", 
                                   ...
                                   ifelse(grepl("lbxp", Variable), "Non-Ortho PCB",
                                          ifelse(grepl("lbxh", Variable), "Non-Ortho PCB", 
                                          "TEQ"))))))

loadings %>% ggplot(aes(x = MR1, y = MR2, color = Group)) + geom_point() + geom_label_repel(aes(label = Variable),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
                  segment.color = 'grey50') + theme_bw() + theme(legend.position = "none") +
  labs(title = "Variable Loadings on First and Second Factors")

loadings %>% ggplot(aes(x = MR1, y = MR3, color = Group)) + geom_point() + geom_label_repel(aes(label = Variable),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
                  segment.color = 'grey50') + theme_bw() + theme(legend.position = "none") +
...
```{r}
plot_loadings <- loadings %>% dplyr::select(-Max) %>% gather(key = "Factor", value = "Loading", -Variable, -Group)

plot_loadings %>% ggplot(aes(x = Variable, y = Loading, fill = Group)) + geom_col() +
  facet_wrap(~Factor) + theme_bw() + theme(legend.position = "bottom",
                                                axis.text.x = element_text(angle = 90, hjust = 1)) +
  coord_flip() + geom_hline(yintercept = 0, size = 0.2) +
  labs(title = "Variable Loadings on All Factors")

plot_loadings %>% ggplot(aes(x = Factor, y = Loading, fill = Factor)) + geom_col(position = "dodge") +
  facet_wrap(~Variable) + 
