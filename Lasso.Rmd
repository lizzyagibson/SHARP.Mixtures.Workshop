...
# Example of extreme lambda values
# create a grid with a range of lambda values
lgrid <- seq(0, 100, length = 100) 

# generate a lasso model using exposure, outcome, and grid of lambda values
lasso.mod = glmnet(x, lny, alpha = 1, lambda = lgrid)
...
studypop <- study_pop %>% mutate(logtelomean = log(telomean))

lm_from_lasso <- lm(logtelomean ~ lbx099la + lbx118la + lbxf03la +lbxpcbla +
...
lm_from_lasso %>% broom::tidy() %>% mutate(
  estimate = round(estimate, 4),
  std.error = round(std.error, 4),
  statistic = round(statistic, 4),
  p.value = round(p.value, 4))

lm_from_lasso %>% broom::glance() %>% mutate(p.value = round(p.value, 5))
```

## Plot Model

```{r}
lm_from_lasso %>% broom::tidy() %>% as.tibble() %>% filter(grepl("la", term)) %>% 
  ...
  labs(y = "Estimate", x = "Pollutant", title = "Association between Pollutants from Lasso and LTL")
