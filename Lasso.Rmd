...
# creating a matrix of predictors as x and outcome as y
...
y = na.omit(data_lasso)$telomean
lny = log(na.omit(data_lasso)$telomean)
...
## Cross-Validation to Find Lambda Value
...
lasso = glmnet(x, y, penalty.factor = c(0, rep(ncol(covs)), alpha = 1))
...
lasso.mod = glmnet(x, y, penalty.factor = c(0, rep(ncol(covs)), alpha = 1, lambda = best_llambda))
...
```{r}
set.seed(2)
elastic = glmnet(x, y, penalty.factor = c(0, rep(ncol(covs)), alpha = 0.5))
...
elastic.mod = glmnet(x, y, penalty.factor = c(0, rep(ncol(covs)), alpha = 0.5, lambda = best_elambda))
