...
Lasso is a dimension-reduction (variable selection) technique that has low flexibility but is instead highly interpretable. This regression technique tends to yield sparse models because of the addition of a penalty term that forces variables with minor contributions to the model to have coefficients of zero.
...
## use penalty.factor to retain certain variables in the model (0 for those that you do not want to shrink)
lasso = glmnet(x, lny, 
...
Elastic net is another dimension-reduction technique and is highly similar to lasso. In fact, when alpha = 1, elastic net and lasso result in the same model. The main difference between elastic net and lasso is that elastic net has an additional penalty term that allows for shrinkage of some variables that are not necessarily reduced to zero. 
...
coef = cbind(as.vector(coef_lasso), as.vector(coef_elnet))
```

The lasso and elastic net techniques resulted in similar lambda values, mean squared errors, variable coefficients, and thus almost identical models. There was one variable that was retained in elastic net and not lasso and it was lbxf08la with a coefficient of 1.19 x 10^-4^. Based strictly on MSE values, lasso would be considered a slightly better model, but not by much. 
