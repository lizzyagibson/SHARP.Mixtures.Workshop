...
save(knots100, file="./BKMR/saved_model/NHANES_knots100.RData")
...
save(fit_gvs_knots100,file="./BKMR/saved_model/bkmr_NHANES_gvs_knots100.RData")
...
pdf(file="./BKMR/figures_pdf/cor_nhanes.pdf", width=12, height=12)
...
save(pred.resp.univar, pred.resp.bivar, pred.resp.bivar.levels, risks.overall, risks.singvar, risks.int, 
     file=paste0("./BKMR/saved_model/", modeltoplot.name,"_plots.RData"))

#load(paste0("./BKMR/saved_model/", modeltoplot.name,"_plots.RData"))

### run and save ggplots for each bkmr model
pdf(file=paste0("./BKMR/figures_pdf/univar_",plot.name,".pdf"), width=15, height=15)
ggplot(pred.resp.univar, aes(z, est, ymin = est - 1.96*se, ymax = est + 1.96*se)) + 
  geom_smooth(stat = "identity") + ylab("h(z)") + facet_wrap(~ variable) 
dev.off()

pdf(file=paste0("./BKMR/figures_pdf/bivar_",plot.name,".pdf"), width=30, height=30)
...
pdf(file=paste0("./BKMR/figures_pdf/bivar_levels_",plot.name,".pdf"), width=30, height=30)
...
pdf(file=paste0("./BKMR/figures_pdf/overallrisks_",plot.name,".pdf"), width=10, height=10)
ggplot(risks.overall, aes(quantile, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd)) +  
  geom_hline(yintercept=00, linetype="dashed", color="gray")+ 
  geom_pointrange() + scale_y_continuous(name="estimate") 
dev.off()

pdf(file=paste0("./BKMR/figures_pdf/singvar_",plot.name,".pdf"), width=5, height=10)
...
pdf(file=paste0("./BKMR/figures_pdf/", "interactplot_ex_",plot.name,".pdf"), width=5, height=10)
