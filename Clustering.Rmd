...
Combine clusters grouping with original data outcome and covariants for clusters = 2

```{r}
hc.clusters.2 = cutree(hc.complete, 2)
hc_health_2 <- as.tibble(cbind(pop, hc.clusters.2))
table(hc.clusters.2)
```

```{r}
health_model_hc2 <- lm(logtelomean ~ as.factor(hc.clusters.2) +
     lbxwbcsi + lbxlypct + lbxmopct + lbxnepct + lbxeopct + lbxbapct +
     as.factor(bmi_cat3) + as.factor(edu_cat) + as.factor(race_cat) + male, data = hc_health_2)

health_model_hc2 %>% broom::tidy() %>% mutate(
  estimate = round(estimate, 4),
  std.error = round(std.error, 4),
  statistic = round(statistic, 4),
  p.value = round(p.value, 4))

health_model_hc2 %>% broom::glance() %>% mutate(p.value = round(p.value, 5))
```

##### Plot

```{r}
health_model_hc2 %>% broom::tidy() %>% as.tibble() %>% filter(grepl("cluster", term)) %>% 
  mutate(term = ifelse(term == "as.factor(hc.clusters.2)2", "Cluster 2", "")) %>%
  ggplot(aes(x = term, y = estimate, color = term,
             ymin = estimate - 1.96*std.error,
             ymax = estimate + 1.96*std.error)) +
  geom_pointrange() + theme_bw() +
...
```{r}
hc.clusters.4 = cutree(hc.complete, 4)
hc_health_4 <- as.tibble(cbind(pop, hc.clusters.4))
table(hc.clusters.4)
```

```{r}
health_model_hc4 <- lm(logtelomean ~ as.factor(hc.clusters.4) +
     lbxwbcsi + lbxlypct + lbxmopct + lbxnepct + lbxeopct + lbxbapct +
     as.factor(bmi_cat3) + as.factor(edu_cat) + as.factor(race_cat) + male, data = hc_health_4)

health_model_hc4 %>% broom::tidy() %>% mutate(
  estimate = round(estimate, 4),
  std.error = round(std.error, 4),
  statistic = round(statistic, 4),
  p.value = round(p.value, 4))

health_model_hc4 %>% broom::glance() %>% mutate(p.value = round(p.value, 5))
```

##### Plot

```{r}
health_model_hc4 %>% broom::tidy() %>% as.tibble() %>% filter(grepl("cluster", term)) %>% 
  mutate(term = ifelse(term == "as.factor(hc.clusters.4)2", "Cluster 2", 
                       ifelse(term == "as.factor(hc.clusters.4)3", "Cluster 3", "Cluster 4"))) %>%
  ggplot(aes(x = term, y = estimate, color = term,
             ymin = estimate - 1.96*std.error,
             ymax = estimate + 1.96*std.error)) +
  geom_pointrange() + theme_bw() +
