...
Ahlam if you can just make this nicer
```{r}
...
plot(km.res$clusters, km.res$PropWithin, type='l', las = 1, ylab = "Proportion of Within over Total SS",

     xlab = "Number of Clusters")

 

plot(km.res$clusters[1:30], km.res$PropWithin[1:30], type='l', las = 1, ylab = "Proportion of Within over Total SS",

     xlab = "Number of Clusters", ylim = c(0,100))
```
...
plot_loadings_pca %>% 
  ...
  labs(title = "Loadings for First 4 Principle Components", x = "Chemicals",
       y = "Loadings")
...
Combine clusters grouping with original data outcome and covariants for K-means = ?
...
health_model_kmeans %>% broom::tidy() %>% as.tibble() %>% filter(grepl("clu.group", term)) %>% 
  mutate(term = ifelse(term == "as.factor(clu.group)5", "",
  ifelse(term == "as.factor(clu.group)4", "MR3", "MR4"))) %>% 
  rbind(., c("MR1", 0, 0, 0, 0)) %>% 
