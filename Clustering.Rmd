...
```
Model with 2 clusters as a categorical variable.
...
```
### Plot models  *** I can't get this to work. The as.factor function is not working and I couldn't figure out why. 
...
health_model_kmeans %>% broom::tidy() %>% as.tibble() %>% filter(grepl("clu.group", term)) %>% 
  ...
  labs(y = "Estimate", x = "Factor", title = "Association between cluster and LTL")
```
