...
summary.km <- matrix(c(km2$totss, km3$totss, km10$totss, km100$totss, km1000$totss, 
                       km2$tot.withinss, km3$tot.withinss, km10$tot.withinss, km100$tot.withinss,
                       km1000$tot.withinss, km2$betweenss, km3$betweenss, km10$betweenss, km100$betweenss,
                       km1000$betweenss), ncol = 3)
...
## Data Visualization ***Need to add description for this

```{r}
plot_km3 <- autoplot(kmeans(na.omit(log.x), 3), 
                     data = na.omit(log.x), 
                     main = "K-means, K = 3", 
                     alpha = 0.8) + 
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank()) 

plot_km10 <- autoplot(kmeans(na.omit(log.x), 10), 
                      data = na.omit(log.x), 
                      main = "K-means, K = 10", 
                      alpha = 0.8) +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank(), 
...
plot_km100 <- autoplot(kmeans(na.omit(log.x), 100), 
                       data = na.omit(log.x), 
                       main = "K-means, K = 100", 
                       alpha = 0.8) + 
  theme(legend.position = "none")

plot_km1000 <- autoplot(kmeans(na.omit(log.x), 1000), 
                        data = na.omit(log.x), 
                        main = "K-means, K = 1000", 
                        alpha = 0.8) + 
