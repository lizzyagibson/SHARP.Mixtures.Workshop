...
FA_betas_confint <- as.data.frame(c(categorical$coefficients["as.factor(Max)MR2"], categorical$coefficients["as.factor(Max)MR3"], categorical$coefficients["as.factor(Max)MR4"])) %>% rownames_to_column() %>% 
  rename("variable" = 1) %>% 
  rename("beta" = 2) %>% 
  mutate('95%CI' = c("0.014, 0.090", "0.003, 0.080","-0.058, 0.021" ),
       'P-value' = c((summary(categorical)$coefficients[2,4]),
                     (summary(categorical)$coefficients[3,4]),
                     (summary(categorical)$coefficients[4,4])))
