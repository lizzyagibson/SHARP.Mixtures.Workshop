...
Kmeans3_betas_confint <- as.data.frame(c(health_model_kmeans3$coefficients["clu.group31"], health_model_kmeans3$coefficients["clu.group32"])) %>% add_rownames() %>% rename("variable" = 1) %>% rename("beta_value" = 2)%>% mutate('95%_CI' = c("0.033-0.132", "0.0162-0.091"))
...
#summary table of beta and confidence intervals for the cluster variables. 

hc3_betas_confint <- as.data.frame(c(health_model_hc3$coefficients["hc.clusters.31"], health_model_hc3$coefficients["hc.clusters.32"])) %>% add_rownames() %>% rename("variable" = 1) %>% rename("beta_value" = 2)%>% mutate('95%_CI' = c("0.007-0.101", "-0.002-0.069"))
