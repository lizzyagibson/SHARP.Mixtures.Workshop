...
PCA_betas_confint <- as.data.frame(c(health_model_pca$coefficients["PC1"], health_model_pca$coefficients["PC2"], health_model_pca$coefficients["PC3"])) %>% rownames_to_column() %>% rename("variable" = 1) %>% rename("beta" = 2) %>% mutate('95%CI' = c("-0.018, -0.005", "-0.009, 0.011","-0.012, 0.017" ),
...
save(PCA_betas_confint, file = "PCA_betas_confint.csv")
