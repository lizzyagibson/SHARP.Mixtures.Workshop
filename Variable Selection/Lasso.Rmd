...
How to get a good lambda value? 

```{r lasso}
set.seed(2)

lam_grid <- .5 ^ (-20:20) ###tuning parameters pre-specified grid.
...
cv.out = cv.glmnet(x, y, alpha = 1) ##implements the cross-validation prediction 10-fold. 

plot(cv.out)
coef(cv.out)

best_lambda = cv.out$lambda.min
best_lambda # we choose a lambda between the dashes lines. 
...
```{r}
is_penalized = c(rep(1, ncol(x[,1:18])), rep(0, ncol(x[,19:36]))) ##assign a value of zero to those variables you would like to keep all the time. Those variables won't be penalized during the lasso process so they will stay in the model. In this case we want to penalize only the POP variables, so we give them a value of 0. 
...
`glmnet` implements the elastic net -- we've set `alpha` to 1 to focus on lasso regression, but could choose `alpha` between 0 and 1. In the following, we can tune alpha and lambda simultaneously. (or set alpha equals to 0 to look into ridge regression)

See plot below, for a fix alpha there is a U-shape curve that gives you what the ultimate value for lambda would be. 
...
control <- trainControl(method = "repeatedcv", ##trainControl is a function in the carrot package. 
                        repeats = 3, 
                        verboseIter = TRUE)
...
elnet = glmnet(x, y, 
               penalty.factor = is_penalized,
               alpha = best_alpha) ## instead of setting alpha = 1, we set it to whatever value yield the best value for lambda. In this case 0.8.
plot(elnet)
...
write_csv(ind, "../Variable Selection/ind_betas.csv")
```
